...
- **Controller routing: folder prefix + lowercased CLASS name.** The framework autoloads every `.ts` under `src/controllers/` and mounts each controller at `/<folder>/<classname>` lowercased (`controllers/public/Impact.ts` with `class Impact` → `/public/impact`) — the FILENAME is irrelevant to routing (it only matters for overriding framework-internal controllers by file-name collision). Use **folders** for grouped areas (`admin/…`, `public/…`). Class names can't contain dashes, so a multi-word kebab-case leaf (`/impact-surveys`) requires a `getHttpPath()` override returning the full path — that's the supported mechanism, not a smell. Name the file after the class (PascalCase). Never export the same controller from two files — autoloading registers both and double-mounts the routes.
