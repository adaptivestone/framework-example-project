...
- Keep the readonly Mongoose schema as the single source of truth; do not copy it
  into a separately maintained document interface
- Use a private `GetModelTypeLiteFromSchema` alias only for explicit `this`
  annotations while the class is being inferred
- Export `GetModelTypeFromClass<typeof Model>` as the complete model type used by
...
class Person extends BaseModel {
  static get modelSchema() {
    return {
      name: { type: String, required: [true, 'person.nameRequired'] },
      labels: { type: [String] },
    } as const;
  }
  ...
  static get modelStatics() {
    return {
      byName(this: PersonAuthoringModel, name: string) {
...
Known literal model names use `app.getModel('Person')` and receive the exact
generated model type. A runtime `string` uses `app.getModelOrThrow(name)` after
validation or authorization; it throws if the model is unavailable instead of
returning `false`.
